...
def define_dataset(logger, opt):
    ...
    if opt['phase'] == 'train' or 'debug' in opt['name']:
        args_val = {}
        for (k, v) in dataset_opt["args"].items():
            if k in ["data_root", "data_flist"]:
                args_val[k] = v.replace('train', 'validate')
            else:
                args_val[k] = v

        dataset_opt["args"] = args_val
