...
def tensor2img(tensor, out_type=np.uint8, min_max=(-1, 1), denorm_fn=None):
    '''
    Converts a torch Tensor into an image Numpy array
    Input: 4D(B,(3/1),H,W), 3D(C,H,W), or 2D(H,W), any range, RGB channel order
    Output: 3D(H,W,C) or 2D(H,W), [0,255], np.uint8 (default)
    '''
    tensor.clamp_(*min_max)  # clamp
    n_dim = tensor.dim()
    if n_dim == 4:
        n_img = len(tensor)
    ...
    if out_type == np.uint8:
        img_np = (denorm_fn(img_np)*255.).round()
        # Important. Unlike matlab, numpy.unit8() WILL NOT round by default.
    return img_np.astype(out_type).squeeze()

def postprocess(images, out_type=np.uint8, min_max=(-1, 1), denorm_fn=None):
    return [tensor2img(image, out_type, min_max, denorm_fn) for image in images]

def normalize_tensor(tensor, min_max=(-1, 1)):
    tensor = tensor.float().clamp_(*min_max)  # clamp
